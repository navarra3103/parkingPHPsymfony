 ...
 */
final class Version20250511174447 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        SQL);
        $this->addSql(<<<'SQL'
            CREATE INDEX IDX_B05D4D94C509A65C ON Visita (Estado_IdEstado)
        SQL);
        $this->addSql(<<<'SQL'
            CREATE INDEX IDX_B05D4D9453C0EC6D ON Visita (Coche_Matricula)
        SQL);
        $this->addSql(<<<'SQL'
            CREATE INDEX IDX_B05D4D9431ECE36D ON Visita (Plaza_IdPlaza)
        SQL);
        $this->addSql(<<<'SQL'
    ...
    public function down(Schema $schema): void
    {
        ...
        SQL);
        $this->addSql(<<<'SQL'
            CREATE TABLE Visita (entrada DATETIME DEFAULT NULL, salida DATETIME DEFAULT NULL, Estado_IdEstado INTEGER NOT NULL, Coche_Matricula VARCHAR(20) NOT NULL, Plaza_IdPlaza INTEGER NOT NULL, PRIMARY KEY(Coche_Matricula, Plaza_IdPlaza), FOREIGN KEY (Estado_IdEstado) REFERENCES Estado (IdEstado) ON UPDATE NO ACTION ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_B05D4D9453C0EC6D FOREIGN KEY (Coche_Matricula) REFERENCES coche (matricula) NOT DEFERRABLE INITIALLY IMMEDIATE, FOREIGN KEY (Plaza_IdPlaza) REFERENCES Plaza (IdPlaza) ON UPDATE NO ACTION ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)
        SQL);
        $this->addSql(<<<'SQL'
